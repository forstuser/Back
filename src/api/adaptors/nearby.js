...
class NearByAdaptor {
  ...
  retrieveNearBy(location, geoLocation, professionIds, reply, userId, request) {
    ...
    return this.filterNearByProfessional(
        professionIds.split('[')[1].split(']')[0].split(',').filter(Boolean),
        userId).then((result) => {
      const finalResult = [];
        ...
        if (origins.length > 0 && destinations.length > 0) {
          googleMapsClient.distanceMatrix({
            ...
            for (let i = 0; i < userWithOrigins.length; i += 1) {
              const tempMatrix = matrix.status === 200 && matrix.json ?
              ...
              if (tempMatrix && tempMatrix.status.toLowerCase() === 'ok') {
                userWithOrigins[i].distanceMetrics = tempMatrix.distance ?
                    ...
                    'km';
                userWithOrigins[i].distance = parseFloat(tempMatrix.distance ?
  ...
  filterNearByProfessional(professionIds, userId) {
    ...
    return this.modals.table_users.findAll({
      ...
      attributes: [
        ...
        'longitude',
        'latitude',
        ['is_enrolled_professional', 'isEnrolled'],
        ['professional_category_id', 'categoryId'],
        ['share_mobile', 'isPhoneAllowed'],
        ['share_email', 'isEmailAllowed'],
