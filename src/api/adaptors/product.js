...
export default class ProductAdaptor {
  ...
  async retrieveEHomeProducts(options, language, limit, offset, sort_by) {
    ...
    let [brands, colours, service_schedules, bills, offline_sellers, product_reviews, categories, accessory_parts] = await Promise.all(
        [
          ...
          billOption.id && billOption.id.length > 0 ?
  ...
  async updateSellerReview(user, seller_id, isOnlineSeller, request) {
    ...
    try {
      let [seller_review_result, user_default_limit_rules, order] = await Promise.all(
          [
            this.modals.seller_reviews.findCreateFind({
              where: whereClause, defaults: defaultClause,
            }), this.categoryAdaptor.retrieveLimitRules({where: {user_id: 1}}),
            order_id ? this.modals.order.findOne({
              where: {id: order_id, seller_id}, attributes: [
                'id', 'delivery_minutes', 'out_for_delivery_time',
                'order_type', 'collect_at_store', 'status_type',
                'created_at', 'updated_at', 'is_modified', 'delayed_delivery',
      ...
      if (!seller_review_result[1]) {
        await seller_review_result[0].updateAttributes(
            JSON.parse(JSON.stringify(
                {review_ratings, review_feedback, review_comments, order_id})));
      }
      if (review_ratings === 5 && order && order.delivery_user_id &&
          order.delayed_delivery && order.order_type === 1) {
        ...
        const delivery_user_review = delivery_user.reviews.find(
            item => item.order_id.toString() === order_id.toString());
        if (delivery_user_review && delivery_user_review.ratings &&
            delivery_user_review.ratings.toString() === '5') {
