...
export default class SellerAdaptor {
  ...
  async retrieveOfferSellers(options, query_options) {
    ...
    const result = await this.modals.sellers.findAll(query_options);
    let sellers = result.map(item => item.toJSON()).
        filter(item => !item.is_logged_out);
    if (sellers.length > 0) {
      if (latitude && longitude) {
        sellers = await this.orderSellerByLocation(
            {
    ...
    const addresses = (await this.userAdaptor.retrieveUserAddresses(
        {
          where: {
            address_type: 1,
            user_id: (user_list || []).map(item => item.id).
                filter(item => item),
          },
  ...
  async createSellerOnInit(seller_detail) {
    ...
    if (result) {
      await this.retrieveOrCreateSellerLoyaltyRules(
          {seller_id: result.seller_id}, {seller_id: result.seller_id});
    }
    return result;
  }
  ...
  async retrieveSellerAddressDifference(address_query_options, query_options) {
    let [seller, user_address] = await Promise.all([
      this.modals.sellers.findOne(query_options),
      this.userAdaptor.retrieveUserAddress(address_query_options)]);
    ...
    if (seller && user_address) {
      ...
      if (latitude && longitude) {
        seller = await this.retrieveSellerByLocation(latitude, longitude,
            city_name, seller);
      }
    }
  ...
  async retrieveSellerProfile(options, query_options) {
    ...
    if (seller) {
      ...
      const [seller_cities, seller_states, seller_locations, seller_reviews] = await Promise.all(
          [
            city_id ?
                this.retrieveSellerCities({id: city_id}) : [],
            state_id ?
  ...
  async retrieveSellerCategories(options) {
    let seller_categories = await this.modals.seller_provider_type.findAll(
        {
          ...
          attributes: [
            ...
            'seller_id', 'provider_type_id', 'category_4_id', 'brand_ids'],
        });
    seller_categories = seller_categories.map(item => item.toJSON());
    let categories_data = [];
    seller_categories.forEach(
        item => {
    ...
    if (categories_data.length > 0) {
        ...
        this.categoryAdaptor.retrieveSellerCategories(
            {
              where: {
                category_id: _.uniq([
                  ...
                  ...categories_data.map(item => item.category_4_id)]),
              }, attributes: ['category_id', 'category_name'],
            }),
        this.retrieveProviderTypes({
          where: {
            id: _.uniq(categories_data.map(item => item.provider_type_id)),
          },
          attributes: ['id', 'title'],
        })]);
      categories_data = categories_data.map(item => {
        ...
        const category_4_detail = sku_categories.find(
            cItem => cItem.category_id === item.category_4_id);
  ...
  async retrieveSellerCashBack(options) {
    let seller_cash_back = await this.modals.cashback_wallet.findAll(
        {where: JSON.parse(JSON.stringify(options)), order: [['id', 'desc']]});
    ...
    const addresses = (await this.userAdaptor.retrieveUserAddresses(
        {
          where: {
            address_type: 1,
            user_id: seller_credits.map(item => item.user_id).
                filter(item => item),
          },
        })).map(item => item.toJSON());
    ...
    const addresses = (await this.userAdaptor.retrieveUserAddresses(
        {
          where: {
            address_type: 1,
            user_id: seller_points.map(item => item.user_id).
                filter(item => item),
          },
        })).map(item => item.toJSON());
  ...
  async orderUserByLocation(latitude, longitude, city, users) {
    ...
    users.forEach((item) => {
      const user = item;
      if (user.addresses) {
        user.geo_location = user.addresses.latitude &&
        user.addresses.longitude &&
        user.addresses.latitude.toString() !== '0' &&
        user.addresses.longitude.toString() !== '0' ?
    ...
    if (origins.length > 0 && destinations.length > 0) {
      ...
      for (let i = 0; i < user_with_location.length; i += 1) {
        if (result.length > 0) {
          ...
          user_with_location[i].distance = tempMatrix &&
          (tempMatrix.distance) ?
    ...
    try {
      if (config.ALLOW_GEO_FILTER &&
        ...
        if (origins.length > 0 && destinations.length > 0) {
          ...
          return distance_filter_required ?
              _.orderBy(final_result.filter(
                  (elem) => (!!elem.distance &&
                      parseFloat(elem.distance) <=
                      config.SELLER_FILTER_DISTANCE)),
                  ['distance'], ['asc']) :
              _.orderBy(final_result, ['distance'], ['asc']);
        }
      }
  ...
  async retrieveOrCreateSellerProviderBrands(
      options, defaults, category_4_id, brand_ids) {
    let seller_provider_type = await this.modals.seller_provider_type.findOne(
        {where: options});
  ...
  async updateAssistedUserReview(options, review) {
    ...
    if (assisted_service_user) {
      ...
      let current_review = assisted_service_users_result.reviews.find(
          item => item.order_id.toString() === review.order_id.toString());
      if (current_review) {
        assisted_service_users_result.reviews = assisted_service_users_result.reviews.map(
            item => {
  ...
  async retrieveOrCreateSellerCredits(
      options, defaults, seller_name, seller_id) {
  ...
  async retrieveOrCreateSellerPoints(
      options, defaults, seller_name, seller_id) {
  ...
  async retrieveSellerLoyaltyRules(options) {
    let loyalty_rules = await this.modals.loyalty_rules.findAll(
        {where: options});
