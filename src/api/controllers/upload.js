    ...
    static uploadWearableImage(request, reply) {
        ...
        if (request.pre.userExist === 0) {
            ...
            return modals.wearables.findOne({
                ...
                if (wearableResult) {
                    const image_code = `${Math.random().toString(36).substr(2, 9)}${(user.id ||
                        user.ID).toString(
                        36)}`;
                ...
                if (result) {
                    fsImpl.readFile(
                        Guid.isGuid(result.job_id) ?
                            ...
                            `jobs/${result.job_id}/${result.copies[0].file_name}`).then(fileResult => {
                        return reply(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition',
    ...
    static retrieveCategoryImage(request, reply) {
        if (!request.pre.forceUpdate) {
            ...
            modals.categories.findOne({
                ...
                return fsImplCategory.readFile(result.category_image_name, 'utf8').then(fileResult => reply(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition',
            ...
            return modals.products.findOne({
                ...
                if (productResult) {
                    ...
                    return fsImplProduct.readFile(
                        `${request.params.id}.${productDetail.file_type}`, 'utf8').then(fileResult => reply(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition',
                        `attachment; filename=${request.params.id}.${productDetail.file_type}`)).catch((err) => {
                        console.log(
        ...
        if (!request.pre.forceUpdate) {
            ...
            return modals.wearables.findOne({
                ...
                if (wearableResult) {
                    ...
                    return fsImplProduct.readFile(
                        `${wearableDetail.image_name}`, 'utf8').then(fileResult => reply(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition',
                        `attachment; filename=${wearableDetail.image_name}`)).catch((err) => {
                        console.log(
    ...
    static retrieveCalendarItemImage(request, reply) {
        if (!request.pre.forceUpdate) {
            const fsImplCategory = new S3FS(
                ...
                config.AWS.ACCESS_DETAILS);
            return fsImplCategory.readFile(`${request.params.id}.png`, 'utf8').then(fileResult => reply(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition',
    ...
    static retrieveBrandImage(request, reply) {
        if (!request.pre.forceUpdate) {
            const fsImplBrand = new S3FS(
                ...
                config.AWS.ACCESS_DETAILS);
            fsImplBrand.readFile(`${request.params.id}.png`, 'utf8').then(fileResult => reply(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition',
                `attachment; filename=${request.params.id}.png`)).catch((err) => {
                console.log(
