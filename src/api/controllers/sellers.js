...
class SellerController {
  ...
  static async updateAssistedServiceUsers(request, reply) {
    ...
    try {
      ...
      const seller_service_types = await sellerAdaptor.retrieveOrCreateAssistedServiceUsers(
          JSON.parse(JSON.stringify({name, mobile_no})),
          JSON.parse(JSON.stringify({name, mobile_no, document_details})),
    ...
    try {
      if (!request.pre.forceUpdate) {
        ...
        const credit_wallet = await sellerAdaptor.retrieveOrCreateSellerCredits(
            {seller_id, id: credit_id, user_id: customer_id}, {job_id});
        if (credit_wallet) {
          return reply.response({
            status: true,
            message: 'Successful',
    ...
    try {
      if (!request.pre.forceUpdate) {
        ...
        const loyalty_wallet = await sellerAdaptor.retrieveOrCreateSellerPoints(
            {seller_id, id: point_id, user_id: customer_id}, {job_id});
        if (loyalty_wallet) {
          return reply.response({
            status: true,
            message: 'Successful',
  ...
  static async deleteAssistedService(request, reply) {
    try {
      if (!request.pre.forceUpdate) {
        const {seller_id, id} = request.params;
        await sellerAdaptor.deleteSellerAssistedService(
            {where: {seller_id, id}});
