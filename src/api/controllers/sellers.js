...
class SellerController {
          ...
          const sellers = await sellerAdaptor.retrieveSellers(
              {
                ...
                attributes: [
                  ...
                  [
                    ...
                    'is_logged_out'], 'is_data_manually_added', [
                    modals.sequelize.literal(
  ...
  static async getOfferSellers(request, reply) {
    ...
    if (request.pre.userExist && !request.pre.forceUpdate) {
      ...
      try {
                ...
                modals.sequelize.literal(
                    `${seller_offer_ids && seller_offer_ids.length > 0 ?
                        `(select count(*) from table_seller_offers as seller_offers where status_type = 1 and seller_offers.id in (${(seller_offer_ids ||
                            []).join(
                            ',')}) and seller_offers.seller_id = "sellers"."id" and seller_offers.end_date >= '${end_date}')` :
                        0}`), 'offer_count'], [
                modals.sequelize.literal(
                    `(select count(*) from table_seller_offers as seller_offers where status_type = 1 and on_sku = true and seller_offers.seller_id = "sellers"."id" and seller_offers.end_date >= '${end_date}')`),
                'sku_offer_count'], [
                modals.sequelize.literal(
                    `(select AVG(seller_reviews.review_ratings) from table_seller_reviews as seller_reviews where seller_reviews.offline_seller_id = "sellers"."id")`),
                'ratings']],
        ...
        return reply.response({
          status: true, result: [],
          message: config.OFFER_NO_SELLER_MSG,
        });
      } catch (err) {
        console.log(`Error on ${new Date()} for user ${user.id ||
        user.ID} is as follow: \n \n ${err}`);
  ...
  static async retrieveSellerOfferList(request, reply) {
    ...
    if (request.pre.userExist && !request.pre.forceUpdate) {
      ...
      try {
        ...
        offset = page_no && page_no > 0 ?
            config.SKU_LIMIT * page_no : offset || 0;
        limit = config.SKU_LIMIT;
        /*if (offer_type === '4') {
        ...
        const $and = brand_ids || sub_category_ids ? modals.sequelize.literal(
            `sku_id in (select id from table_sku_global as sku where ${brand_ids ?
                `sku.brand_id in (${brand_ids}) ${sub_category_ids ?
                    `and sku.sub_category_id in (${sub_category_ids})` :
                    ''}` : `${sub_category_ids ?
                    `sku.sub_category_id in (${sub_category_ids})` :
                    ''}`})`) : undefined;
        let result = await sellerAdaptor.retrieveSellerOffersForConsumer(
            $and ? {seller_id, offer_type, $and} : {seller_id, offer_type},
            limit, offset);
        result = offer_type === '1' ? _.orderBy(result.filter(
            offerItem => (offerItem.on_sku && offerItem.offer_discount >
                0)), ['sub_category_id']) : result;
    ...
    if (request.pre.userExist && !request.pre.forceUpdate) {
      ...
      try {
        ...
        (user_index_data.my_seller_ids || []).length > 0 ?
            JSON.parse(
                JSON.stringify({$notIn: user_index_data.my_seller_ids})) :
            undefined;
  ...
  static async updateSellerBasicDetail(request, reply) {
    ...
    try {
      ...
      basic_details.pay_online = !pay_online && pay_online !== false ?
  ...
  static async linkCustomers(request, reply) {
    ...
    try {
      ...
      seller_data.customer_ids = (seller_data.customer_ids || []).map(item => {
        if (item.customer_id) {
          if (item.customer_id === parseInt(customer_id)) {
            ...
            item.is_credit_allowed = !is_credit_allowed && is_credit_allowed !==
            false ?
                item.is_credit_allowed || false : is_credit_allowed;
            item.credit_limit = !credit_limit && credit_limit !== 0 ?
                item.credit_limit || 0 :
                credit_limit;
            return item;
          }
      ...
      seller_data.customer_ids = _.uniqBy(seller_data.customer_ids,
          'customer_id');
  ...
  static async inviteCustomers(request, reply) {
    ...
    try {
      ...
      seller_data.customer_ids = (seller_data.customer_ids || []).map(item => {
        if (item.customer_id) {
          if (item.customer_id === parseInt(user_data.id)) {
            customer_id_exist = true;
            item.is_credit_allowed = is_credit_allowed ||
      ...
      seller_data.customer_invite_detail.push(
          {
            customer_id: parseInt(user_data.id),
            invited_date: moment().utcOffset(330),
          });
      seller_data.customer_invite_detail = _.uniqBy(
          seller_data.customer_invite_detail, 'customer_id');
      replyObject.seller_detail = JSON.parse(
...
Download Now: http://bit.ly/binbill`;
  ...
  static async updateSellerProviderTypeBrands(request, reply) {
    ...
    try {
      const {seller_id} = request.params || {};
      const seller_provider_types = await Promise.all(
          (request.payload.provider_type_detail.length ?
              request.payload.provider_type_detail :
              [{brand_ids: [], category_4_id: 0}]).map(item => {
            const {provider_type_id, sub_category_id, category_brands, category_4_id, brand_ids} = item;
            return sellerAdaptor.retrieveOrCreateSellerProviderBrands(
                JSON.parse(
                    JSON.stringify(
                        {provider_type_id, seller_id, sub_category_id})),
                JSON.parse(JSON.stringify({
  ...
  static async updateSellerOffers(request, reply) {
    ...
    try {
      ...
      if (brand_offer_id) {
        brand_offer = modals.brand_offers.findOne(
            {where: {id: brand_offer_id}});
        ...
        const result = await sellerAdaptor.retrieveSellerWalletDetail({
          where: {
            ...
            'amount', 'status_type', 'is_paytm', 'created_at', [
              ...
              modals.sequelize.literal(
                  '(select mobile_no from users where users.id = "seller_wallet".user_id)'),
              'mobile_no',
  ...
  static async retrieveSellerOffers(request, reply) {
    try {
      if (!request.pre.forceUpdate) {
        ...
        let seller_offers = await sellerAdaptor.retrieveSellerOffers({
          ...
          attributes: [
            ...
            'offer_discount', 'seller_mrp', [
              ...
              'sku_title'], [
              modals.sequelize.literal(
                  '(select title from table_brand_offers as brand_offer where brand_offer.id = seller_offers.brand_offer_id)'),
              'brand_offer_title'], 'offer_type', [
              modals.sequelize.literal(
        ...
        }).filter(item => !item.on_sku || item.offer_type !== 1 ||
            (item.offer_type === 1 && item.on_sku && item.sku.offer_discount >
                0));
      ...
      if (!request.pre.forceUpdate) {
        ...
        const credits = await sellerAdaptor.retrieveSellerCreditsPerUser({
          where: {seller_id}, include: {
            ...
            modals.sequelize.literal('"user"."id"')], attributes: [
            'user_id', 'transaction_type',
  ...
  static async retrieveSellerConsumers(request, reply) {
    try {
      if (!request.pre.forceUpdate) {
        ...
        linked_only = (linked_only && linked_only.toString().toLowerCase() ===
            'true');
        const {seller_customers, customer_count, last_page, loyalty_rule} = await sellerAdaptor.retrieveSellerConsumers(
            {seller_id, mobile_no, offer_id, user_status_type, page_no});
        return reply.response({
          status: true, message: 'Successful', result: offer_id ?
              ...
              mobile_no || !linked_only ? seller_customers :
                  seller_customers.filter(item => item.linked), loyalty_rule,
    ...
    try {
      if (!request.pre.forceUpdate) {
        ...
        return reply.response({
          status: true,
          message: 'Successful',
          result: _.orderBy(seller_customers, ['total_transactions'], ['desc']),
          total_transactions: _.sumBy(seller_customers, 'total_transactions') ||
              0,
          forceUpdate: request.pre.forceUpdate,
        });
      } else {
      ...
      if (!request.pre.forceUpdate) {
        ...
        return reply.response({
          ...
          result: await sellerAdaptor.retrieveSellerCustomerDetail(seller_id,
              customer_id, mobile_no), forceUpdate: request.pre.forceUpdate,
        });
      }
