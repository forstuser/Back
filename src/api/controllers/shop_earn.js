    ...
    if (!request.pre.forceUpdate) {
      ...
      try {
        ...
        if (sku_item.sub_category_id) {
          ...
          const sku_ratio = sku_measurement.mrp && sku_measurement.mrp > 0 ?
              sku_measurement.measurement_value / sku_measurement.mrp :
              -1;
          ...
          sku_result.sort((a, b) => {
            ...
            if (a.sku_measurements.length > 0 && b.sku_measurements.length >
                0) {
              if (a.sku_measurements.length >= b.sku_measurements.length) {
                a.sku_measurements.forEach((item, index) => {
                  ...
                  a_sku_ratio += item.mrp > 0 ?
                      item.measurement_value / item.mrp : -1;
                  if (b_item) {
                    b_sku_ratio += b_item.mrp > 0 ?
                        b_item.measurement_value / b_item.mrp : -1;
                  }
                });
              } else {
                b.sku_measurements.forEach((item, index) => {
                  const a_item = a.sku_measurements[index];
                  b_sku_ratio += item.mrp > 0 ?
                      item.measurement_value / item.mrp : -1;
                  if (a_item) {
                    a_sku_ratio += a_item.mrp > 0 ?
                        a_item.measurement_value / a_item.mrp : -1;
                  }
                });
              }
              ...
              return (Math.abs(sku_ratio - a_sku_ratio) -
                  Math.abs(sku_ratio - b_sku_ratio));
            }
        ...
        const seller_cashback = await jobAdaptor.retrieveSellerCashBack({
          where: {seller_id, id, status_type: 13}, attributes: [
            'job_id', 'id', 'user_id', 'amount', 'status_type', 'created_at', [
              ...
              'user_name'], [
              modals.sequelize.literal(
                  `(Select sum(purchase_cost) from consumer_products as "product" where "product".job_id in (Select job_id from table_cashback_jobs as job where job.id = cashback_wallet.job_id))`),
              'purchase_cost'], [
              modals.sequelize.literal(
                  `(Select id from table_orders as "order" where "order".job_id = cashback_wallet.job_id)`),
              'order_id']],
        });
        if (seller_cashback) {
          ...
          const [cash_back_job, user_cash_back_month, user_cash_back_day, user_limit_rules, user_default_limit_rules, seller_loyalty_rules] = await Promise.all(
              [
          ...
          if (seller_loyalty_rules.order_value &&
              ...
              !seller_cashback.order_id) {
            const seller_points = await sellerAdaptor.retrieveOrCreateSellerPoints(
                {seller_id, user_id: seller_cashback.user_id}, {
                  amount: Math.floor(
                      seller_cashback.purchase_cost /
                      seller_loyalty_rules.order_value *
                      seller_loyalty_rules.points_per_item),
                  transaction_type: 1,
                  status_type: 16,
                  user_id: seller_cashback.user_id,
                  seller_id,
