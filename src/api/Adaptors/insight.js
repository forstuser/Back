...
class InsightAdaptor {
    ...
    prepareInsightData(user, request) {
        ...
        return this.prepareCategoryData(user, {}).then((result) => {
            ...
            const categoryData = !(minDate || maxDate) ? {
                weeklyData: result.map((item) => {
                    const expenses = item.expenses.filter(
                        (item) => moment.utc(item.purchaseDate, moment.ISO_8601).valueOf() >=
                ...
                monthlyData: result.map((item) => {
                    const expenses = item.expenses.filter(
                        (item) => moment.utc(item.purchaseDate, moment.ISO_8601).valueOf() >=
                            ...
                            moment.utc(item.purchaseDate, moment.ISO_8601).valueOf() <= moment.utc().valueOf());
                    const totalAmount = shared.sumProps(expenses,
                        'value');
                    const totalTax = shared.sumProps(expenses,
                        'taxes');
            ...
            if (minDate || maxDate) {
                categoryData.customDateData = _.chain(categoryData.customDateData).map((elem) => {
            ...
            categoryData.overallData = _.chain(categoryData.overallData).map((elem) => {
                elem.totalAmount = parseFloat(elem.totalAmount);
                return elem;
            }).orderBy(['totalAmount', 'cName'], ['desc', 'asc']).map((elem) => {
                elem.totalAmount = elem.totalAmount.toString();
            ...
            return {
                ...
                message: 'Insight restore successful',
                categoryData,
                weekStartDate: moment.utc().subtract(6, 'd').startOf('d').format(dateFormatString),
                monthStartDate: moment.utc(monthStartDay, moment.ISO_8601).format(dateFormatString),
                weekEndDate: moment.utc().format(dateFormatString),
                monthLastDate: moment.utc(monthLastDay, moment.ISO_8601).format(dateFormatString),
                yearStartDate: moment.utc(yearStartDay, moment.ISO_8601).format(dateFormatString),
                yearEndDate: moment.utc(yearLastDay, moment.ISO_8601).format(dateFormatString),
        ...
        return this.prepareCategoryData(user, {category_id: masterCategoryId}).then((result) => {
            ...
            productList.map((item) => {
                ...
                const index = distinctInsight.findIndex(
                    distinctItem => (moment.utc(distinctItem.purchaseDate).startOf('day').diff(moment.utc(expense.purchaseDate, moment.ISO_8601).startOf('day'),
                        'days', true) === 0));
                if (index === -1) {
                    distinctInsight.push({
                        ...
                        monthId: moment.utc(expense.purchaseDate, moment.ISO_8601).month() + 1,
            ...
            const distinctInsightTemp = distinctInsight.map((item) => {
                ...
                const weekIndex = distinctInsightWeekly.findIndex(
                    distinctItem => (distinctItem.week === weekItem.week) && (distinctItem.month === weekItem.month) &&
            ...
            const productListWeekly = productList.filter(
                ...
                item.purchaseDate = moment.utc(item.purchaseDate, moment.ISO_8601).startOf('days');
                return item;
            });
