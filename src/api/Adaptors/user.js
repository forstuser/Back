...
class UserAdaptor {
	...
	loginOrRegister (whereObject, defaultObject) {
		return this.modals.users.findOrCreate({
			where: whereObject,
	...
	retrieveUserProfile (user, request) {
		return this.modals.users.findById(user.ID, {
			attributes: [
				...
				'email_verified',
				'email_secret',
				[this.modals.sequelize.fn('CONCAT', 'consumer/', this.modals.sequelize.col('id'), '/images'), 'imageUrl']
			],
			include: [{
				model: this.modals.userAddress,
				where: {
					status_type: 1
				},
				attributes: ['address_type', 'address_line_1', 'address_line_2', 'city', 'state', 'pin', 'latitude', 'longitude']
			}]
		}).then((result) => {
	...
	updateUser (user, request, reply) {
		...
		return this.modals.users.update(userUpdates, {
			where: {
				ID: user.ID
			}
		}).then(() => {
