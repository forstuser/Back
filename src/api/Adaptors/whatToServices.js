...
export default class WhatToServiceAdaptor {
    ...
    retrieveStateMealItems(options, limit, offset) {
        return Promise.try(() => Promise.all([
            ...
            this.retrieveUserMeals(
                {where: {user_id: options.user_id, status_type: 1}})])).spread((stateMeals, userMeals) => {
    ...
    retrieveUserMealItems(options, limit, offset) {
        return Promise.try(() => this.retrieveUserMeals(
            ...
            const currentDateItem = mealDates.find(
                (dateItem) => moment(dateItem.selected_date, moment.ISO_8601).isSame(options.current_date ?
                    moment(options.current_date, moment.ISO_8601) :
                    moment(), 'day'));
            const futureDateItem = mealDates.find(
                (dateItem) => moment(dateItem.selected_date, moment.ISO_8601).isAfter(options.current_date ?
                    moment(options.current_date, moment.ISO_8601) :
                    moment(), 'day'));
            mealDates = _.orderBy(
                (userMeal.meal_dates || []), ['selected_date'],
            ...
            const lastDateItem = mealDates.find(
                (dateItem) => moment(dateItem.selected_date, moment.ISO_8601).isBefore(options.current_date ?
                    moment(options.current_date, moment.ISO_8601) :
                    moment(), 'day'));
            if (currentDateItem) {
            ...
            const mealList = mealItemList.filter(
                (item) => item.current_date &&
                    (moment(item.current_date, moment.ISO_8601).isSame(options.current_date ?
                        moment(options.current_date, moment.ISO_8601) :
                        moment(), 'day') ||
                        moment(item.future_date, moment.ISO_8601).isSame(options.current_date ?
                            moment(options.current_date, moment.ISO_8601) :
                            moment(), 'day') ||
                        moment(item.last_date, moment.ISO_8601).isSame(options.current_date ?
                            moment(options.current_date, moment.ISO_8601) :
                            moment(), 'day')));
            const previousMealList = mealItemList.filter(
                (item) => (item.current_date &&
                    moment(item.current_date, moment.ISO_8601).isBefore(options.current_date ?
                        moment(options.current_date, moment.ISO_8601) :
                        moment(), 'day') ||
                    moment(item.current_date, moment.ISO_8601).isAfter(options.current_date ?
                        moment(options.current_date, moment.ISO_8601) :
                        moment(), 'day')));
    ...
    addUserMealItem(options) {
        return Promise.try(() => this.modals.meals.bulkCreate(options.meal_items,
            {returning: true})).then((mealResult) => {
