    ...
    const refuelingResult = await this.modals.refueling.findAll({
      ...
      attributes: [
        'id', 'product_id', [
          ...
          'main_category_id'], 'job_id', 'document_number', [
          this.modals.sequelize.literal('"product"."product_name"'),
          'product_name'], ['purchase_cost', 'value'],
        ['purchase_taxes', 'taxes'], 'document_date',
        'effective_date', 'odometer_reading', [
          this.modals.sequelize.fn('CONCAT', 'products/',
              this.modals.sequelize.literal('"product_id"')),
          'productURL'], 'copies', 'user_id', 'fuel_type', 'fuel_quantity'],
    ...
    try {
      ...
      if (result) {
        await Promise.all([
          ...
          this.modals.refueling.destroy({where: {id, user_id}}),
          result.copies && result.copies.length > 0 ?
              this.modals.jobCopies.update(
                  {status_type: 3, updated_by: user_id},
...
export default RefuelingAdaptor;