...
function retrieveDaysInsight(distinctInsight) {
    ...
    distinctInsight.map((item) => {
        const currentDate = moment.utc(item.purchaseDate, moment.ISO_8601).startOf('day');
    ...
    return allDaysInWeek.map(weekItem => ({
        ...
        purchaseDay: moment.utc(weekItem.purchaseDate, moment.ISO_8601).format('ddd'),
    }));
}
...
export function preparePaymentDetails(parameters) {
    ...
    const daysInMonth = moment().isoWeekdayCalc(monthStartDate, month_end_date, selected_days);
...
export function monthlyPaymentCalc(parameters) {
    ...
    const monthDiff = moment().startOf('months').diff(moment(effectiveDate, moment.ISO_8601).startOf('months'), 'months');
    console.log('\n\n\n\n\n\n\n\n\n\n monthdiff:', monthDiff);
    const monthArr = [];
