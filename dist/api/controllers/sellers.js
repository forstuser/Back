    ...
    try {
      if (!request.pre.forceUpdate) {
        ...
        return reply.response({
          ...
          total_cashback: Math.round(assigned_cashback || 0) - Math.round(redeemed_cashback || 0),
          forceUpdate: request.pre.forceUpdate
        });
      } else {
        return reply.response({
    ...
    try {
      if (!request.pre.forceUpdate) {
        ...
        const result = await sellerAdaptor.retrieveSellerLoyaltyPointsPerUser({
          ...
          }, attributes: ['title', 'id', 'description', 'transaction_type', 'amount', 'created_at'],
          order: [['created_at', 'desc']]
        });
        return reply.response({
          status: true,
