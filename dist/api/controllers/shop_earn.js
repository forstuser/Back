      ...
      try {
        ...
        if (main_category_id || category_id || brand_ids || sub_category_ids || measurement_values || measurement_types || bar_code || title || limit || id || seller_id) {
          ...
          if (request.query && request.query.title) {
            const matching_skus = sku_result.sku_items.filter(item => (item.sub_category_name || '').toLowerCase() === request.query.title.toLowerCase());
            const distinct_skus = sku_result.sku_items.filter(item => (item.sub_category_name || '').toLowerCase() !== request.query.title.toLowerCase());
  ...
  static async getWalletDetail(request, reply) {
    ...
    if (request.pre.userExist && !request.pre.forceUpdate) {
      try {
        ...
        return reply.response({
          status: true,
          total_cashback: _lodash2.default.sumBy(result.filter(item => item.transaction_type === 1), 'amount') - _lodash2.default.sumBy(result.filter(item => item.status_type === 13 && item.is_paytm || item.status_type === 14 || item.transaction_type === 2),
