...
class OrderController {
        ...
        if (order.order_details.length > 0) {
          ...
          if (result) {
            await notificationAdaptor.notifyUserCron({
              ...
              payload: {
                order_id: result.id, order_type: result.order_type, collect_at_store,
                status_type: result.status_type, user_id,
                title: `Item returned.`,
                description: `An Item, '${sku_item.title}${sku_item.sku_measurement ? sku_item.sku_measurement.measurement_value + ' ' + sku_item.sku_measurement.measurement_acronym : ''}' has been returned by ${result.user.name || ''}.`, notification_type: 1, notification_id: result.id
              }
            });
          } else {
  ...
  static async paymentPostBackUrl(request, reply) {
    try {
      ...
      if (payment_data) {
        ...
        [payment_data, payment_info] = await Promise.all([orderAdaptor.retrieveOrUpdatePaymentDetails({ where: { id: payment_data.id } }, {
  ...
  static async retrievePaymentStatus(request, reply) {
    try {
      ...
      if (payment_detail && payment_detail.status_type !== 13 && payment_detail.status_type !== 8 && payment_detail.status_type !== 4) {
        ...
        return reply.response({ status: true, status_type: payment_detail.status_type });
      }
  ...
  static async retrieveDigitalBillDetail(expense_id, order_id) {
    let [expense_sku, payment_item, order] = await Promise.all([shopEarnAdaptor.retrieveUserSKUExpenses({
      where: { expense_id },
      attributes: ['sku_id', [modals.sequelize.literal('(select title from table_sku_global as sku where sku.id = expense_sku_items.sku_id)'), 'title'], [modals.sequelize.literal('(select hsn_code from table_sku_global as sku where sku.id = expense_sku_items.sku_id)'), 'hsn_code'], [modals.sequelize.literal('(select sub_category_id from table_sku_global as sku where sku.id = expense_sku_items.sku_id)'), 'sub_category_id'], [modals.sequelize.literal('(select priority_index from table_sku_global as sku where sku.id = expense_sku_items.sku_id)'), 'priority_index'], [modals.sequelize.literal('(select tax from table_sku_measurement_detail as sku_measure where sku_measure.id = expense_sku_items.sku_measurement_id)'), 'tax'], [modals.sequelize.literal('(select pack_numbers from table_sku_measurement_detail as sku_measure where sku_measure.id = expense_sku_items.sku_measurement_id)'), 'pack_numbers'], [modals.sequelize.literal('(Select acronym from table_sku_measurement as measurement where measurement.id = (select measurement_type from table_sku_measurement_detail as sku_measure where sku_measure.id = expense_sku_items.sku_measurement_id limit 1))'), 'measurement_acronym'], [modals.sequelize.literal('(select measurement_value from table_sku_measurement_detail as sku_measure where sku_measure.id = expense_sku_items.sku_measurement_id limit 1)'), 'measurement_value'], [modals.sequelize.literal('(select bar_code from table_sku_measurement_detail as sku_measure where sku_measure.id = expense_sku_items.sku_measurement_id limit 1)'), 'bar_code'], 'sku_measurement_id', 'quantity', 'created_at', 'available_cashback', 'selling_price', 'job_id']
    }), orderAdaptor.retrieveOrUpdatePaymentDetails({ where: { order_id } }), orderAdaptor.retrieveOrderDetail({ where: { id: order_id }, attributes: ['seller_id'] })]);
    if (expense_sku) {
      let [seller, cash_back] = await Promise.all([sellerAdaptor.retrieveSellerDetail({
        where: { id: order.seller_id }, attributes: ['seller_name', 'contact_no', 'address', [modals.sequelize.literal('(Select state_name from table_states as state where state.id = sellers.state_id)'), 'state_name'], [modals.sequelize.literal('(Select name from table_cities as city where city.id = sellers.city_id)'), 'city_name'], [modals.sequelize.literal('(Select name from table_localities as locality where locality.id = sellers.locality_id)'), 'locality_name'], [modals.sequelize.literal('(Select pin_code from table_localities as locality where locality.id = sellers.locality_id)'), 'pin_code'], 'latitude', 'longitude', 'url', 'email', 'gstin']
      ...
      payment_item.cash_back = cash_back || 0;
