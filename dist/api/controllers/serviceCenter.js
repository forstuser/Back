...
class ServiceCenterController {
  ...
  static retrieveServiceCenters(request, reply) {
    ...
    if ((request.pre.userExist || isWebMode) && !request.pre.forceUpdate) {
      ...
      Promise.all([serviceCenterAdaptor.retrieveServiceCenters(whereClause), brandAdaptor.retrieveCategoryBrands(brandDetailOption), brandAdaptor.retrieveBrandById(brandId, brandDetailOption)]).then(result => {
        ...
        if (result[0].length > 0) {
          ...
          if (origins.length > 0 && destinations.length > 0) {
            return _google2.default.distanceMatrix(origins, destinations).then(result => {
