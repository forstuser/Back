...
const ALLOWED_FILE_TYPES = ['txt', 'pdf', 'doc', 'docx', 'rtf', 'xls', 'xlsx', 'png', '3gp', 'bmp', 'jpg', 'jpeg', 'heif', 'heic', 'avi', 'flv', 'wmv', 'mov', 'mp4', 'ts'];
...
class UploadController {
  ...
  static async uploadSKUImage(request, reply) {
    if (request.payload) {
      try {
        ...
        if (skuResult) {
          const image_code = `${Math.random().toString(36).substr(2, 9)}${request.params.id.toString(36)}`;
          ...
          const fsImplProduct = new _s3fs2.default(`${_main2.default.AWS.S3.BUCKET}/${_main2.default.AWS.S3.SKU_IMAGE}`, _main2.default.AWS.ACCESS_DETAILS);
          await fsImplProduct.writeFile(image_name, fileData._data, { ContentType: _mimeTypes2.default.lookup(image_name) });
    ...
    try {
      ...
      const fileUploadPromises = fileData.map(async (elem, index) => {
        ...
        if (seller_data.seller_details) {
          ...
          if (image_index) {
            image_detail = type.toString() === '1' && basic_details ? isNaN(image_index) ? basic_details.documents.find(item => item.index === image_index) : basic_details.documents[image_index] : type.toString() === '2' && business_details ? isNaN(image_index) ? business_details.documents.find(item => item.index === image_index) : business_details.documents[image_index] : (type.toString() === '3' || type.toString() === '5') && assisted_type_images ? isNaN(image_index) ? assisted_type_images.find(item => item.index === image_index) : assisted_type_images[image_index] : type.toString() === '4' && offers ? isNaN(image_index) ? offers.find(item => item.index === image_index) : offers[image_index] : undefined;
          }
          if (type.toString() === '1' && basic_details && basic_details.documents && basic_details.documents.length > 0) {
      ...
      fileResult.forEach((elem, index) => {
        if (image_index) {
          if (type.toString() === '1') {
            ...
            business_details = seller_details.business_details || { documents: [] };
  ...
  static async uploadResponse(parameters) {
    ...
    const fixed_cash_backs = (_main2.default.FIXED_CASH_BACK || '').split('||');
    const replyResult = {
      status: true,
      job_id: jobResult.id,
      message: 'Uploaded Successfully',
      billResult: copyData,
      fixed_cashback: jobResult.cashback_job_count > 0 ? parseFloat((fixed_cash_backs[1] || 0).toString()) : parseFloat((fixed_cash_backs[0] || 0).toString())
    };
  ...
  static async retrieveSellerImages(request, reply) {
    if (!request.pre.forceUpdate) {
      try {
        let { id, type, index } = request.params || {};
        const seller_image_types = _main2.default.SELLER_IMAGE_TYPE.split(',');
        ...
        if (seller_data.seller_details) {
          if (type.toString() === '1') {
            const document = isNaN(index) ? seller_data.seller_details.basic_details.documents.find(item => item.index === index) : seller_data.seller_details.basic_details.documents[index];
            file_name = (document || {}).file_name;
          } else if (type.toString() === '2') {
            const document = isNaN(index) ? seller_data.seller_details.business_details.documents.find(item => item.index === index) : seller_data.seller_details.business_details.documents[index];
            file_name = (document || {}).file_name;
          } else if (type.toString() === '5' || type.toString() === '3') {
            const document = isNaN(index) ? seller_data.seller_details.assisted_type_images.find(item => item.index === index) : seller_data.seller_details.assisted_type_images[index];
            file_name = (document || {}).file_name;
            type = 3;
          } else {
            const document = isNaN(index) ? seller_data.seller_details.offers.find(item => item.index === index) : seller_data.seller_details.offers[index];
            file_name = (document || {}).file_name;
          }
          if (file_name) {
  ...
  static async retrieveAssistedTypeImages(request, reply) {
    if (!request.pre.forceUpdate) {
      try {
        ...
        if (file_name) {
          const fileResult = await fsImpl.readFile(`sellers/service_types/${file_name}`, 'utf8');
      ...
      try {
        ...
        if (seller_data.seller_details) {
          ...
          seller_data.seller_details.assisted_type_images.documents.forEach(document => {
            file_name = (document || {}).file_name;
            seller_image_update.push(fsImpl.unlink(`sellers/${id}/${seller_image_types[3]}/${file_name}`));
          });
  ...
  static async retrieveSKUImage(request, reply) {
    if (!request.pre.forceUpdate) {
      ...
      let sku_detail;
      const fsImplSKU = new _s3fs2.default(`${_main2.default.AWS.S3.BUCKET}/${_main2.default.AWS.S3.SKU_IMAGE}${request.params.file_type ? `/${request.params.file_type}` : ''}`, _main2.default.AWS.ACCESS_DETAILS);
      try {
        ...
        if (sku_detail) {
          ...
          if (measurement_id) {
            const fileResult = await fsImplSKU.readFile(`${id}-${measurement_id}.png`, 'utf8');
            return reply.response(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition', `attachment; filename=${id}-${measurement_id}.png`);
          } else if (sku_detail.image_code) {
            const fileResult = await fsImplSKU.readFile(`${request.params.id}.png`, 'utf8');
            return reply.response(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition', `attachment; filename=${request.params.id}.png`);
          } else if (sku_detail.brand_id) {
            const fsImplBrand = new _s3fs2.default(`${_main2.default.AWS.S3.BUCKET}/${_main2.default.AWS.S3.BRAND_IMAGE}${request.params.file_type ? `/${request.params.file_type}` : ''}`, _main2.default.AWS.ACCESS_DETAILS);
            const fileResult = await fsImplBrand.readFile(`${sku_detail.brand_id}.png`, 'utf8');
            return reply.response(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition', `attachment; filename=${sku_detail.brand_id}.png`);
          }
        }
        ...
        console.log(`Error on ${new Date()} retrieving sku image is as follow: \n \n ${err}`);
        try {
          if (measurement_id) {
            const fileResult = await fsImplSKU.readFile(`${request.params.id}.png`, 'utf8');
            return reply.response(fileResult.Body).header('Content-Type', fileResult.ContentType).header('Content-Disposition', `attachment; filename=${request.params.id}.png`);
  ...
  static async uploadSellerOfferImages(request, reply) {
    if (!request.pre.forceUpdate) {
      try {
        ...
        if (seller_offer) {
          (seller_offer || {}).document_details = (seller_offer || {}).document_details && (seller_offer || {}).document_details.length > 0 ? (seller_offer || {}).document_details : [{
            file_name: undefined,
            index: 0,
            file_type: undefined,
          ...
          if (seller_offer.document_details && seller_offer.document_details.length > 0) {
            document = !isNaN(index) ? (seller_offer || {}).document_details[index] : (seller_offer || {}).document_details.find(item => item.index && item.index === index);
          }
          let file_name = (document || {}).file_name;
          const fieldNameHere = request.payload.fieldNameHere;
          let filteredFileData = fieldNameHere || request.payload.filesName || request.payload.file;
          if (filteredFileData) {
            ...
            await fsImpl.writeFile(`sellers/${(seller_offer || {}).seller_id}/${seller_image_types[4]}/${document.file_name}`, elem._data, { ContentType: _mimeTypes2.default.lookup(fileName) || 'image/jpeg' });
            document.index = file_index;
            seller_offer.document_details = (seller_offer.document_details || [document]).length > 0 ? seller_offer.document_details || [document] : [document];
            return reply.response({
              status: true,
          ...
          return reply.response({ status: false, message: 'No a valid offer in request' }); //, forceUpdate: request.pre.forceUpdate});
        }
      } catch (err) {
        console.log(`Error on ${new Date()} retrieving fact image is as follow: \n \n ${err}`);
        modals.logs.create({
      ...
      try {
        ...
        if (assisted_user) {
          ...
          if (filteredFileData) {
            ...
            document.type = 5;
            assisted_user.profile_image_detail = assisted_user.profile_image_detail || document;
          ...
          return reply.response({ status: false, message: 'No assisted user in request' });
        }
      } catch (err) {
        console.log(`Error on ${new Date()} retrieving fact image is as follow: \n \n ${err}`);
        modals.logs.create({
