...
class ShopEarnAdaptor {
    ...
    try {
      ...
      if (seller_id) {
        sku_measurement_attributes.push([this.modals.sequelize.literal(`(select offer_discount from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}')`), 'offer_discount'] /*, [
                                                                                                                                                                                                                                                                                                                                                                        ...
                                                                                                                                                                                                                                                                                                                                                                        format('YYYY-MM-DD')}'))`),
                                                                                                                                                                                                                                                                                                                                                                        'offer_acronym']*/, [this.modals.sequelize.literal(`(select seller_mrp from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id})`), 'seller_mrp']);
      }
  ...
  async retrieveSellerSKUs(options) {
    try {
      ...
      measurement_values = (measurement_values || '').trim().split(',').filter(item => !!item);
      measurement_types = (measurement_types || '').trim().split(',').filter(item => !!item);
      const sku_measurement_attributes = ['id', 'pack_numbers', 'measurement_type', 'measurement_value', 'sku_id', 'mrp', [this.modals.sequelize.literal('(select acronym from table_sku_measurement as measurement where measurement.id = "sku_measurement".measurement_type)'), 'measurement_acronym'], 'bar_code', 'cashback_percent'];
      const sku_attributes = [[this.modals.sequelize.literal('(select category_name from categories as category where category.category_id = sku.sub_category_id)'), 'sub_category_name'], 'brand_id', 'category_id', 'sub_category_id', 'main_category_id', 'title', 'id', 'priority_index'];
  ...
  async createUserSKUWishList(reply, request, user_id) {
    ...
    try {
      const [userSKUWishList, seller_sku] = await _bluebird2.default.all([this.retrieveUserSKUs({
        ...
        }, attributes: ['sku_id', 'sku_measurement_id', 'offer_discount', 'title', 'offer_type', [this.modals.sequelize.literal(`(Select title from table_brand_offers as brand_offer where brand_offer.id = seller_offers.brand_offer_id limit 1)`), 'brand_offer_title'], [this.modals.sequelize.literal(`(Select acronym from table_sku_measurement as measure where measure.id = seller_offers.sku_measurement_type)`), 'offer_acronym']]
      }) : undefined]);
      ...
      if (id) {
        wishlist_items = (wishlist_items && wishlist_items.length > 0 ? wishlist_items.map(item => {
          ...
          if (item.id === id && item_measurement_type === measurement_type && item_measurement_value === measurement_value) {
            ...
            item.offer_title = offer_title || (seller_sku || {}).title || (seller_sku || {}).brand_offer_title;
            item.offer_type = offer_type || (seller_sku || {}).offer_type;
            item.offer_acronym = offer_acronym || (seller_sku || {}).offer_acronym;
            item.seller_id = seller_id;
        ...
        if (!payload_added) {
          ...
          request.payload.offer_title = offer_title || (seller_sku || {}).title || (seller_sku || {}).brand_offer_title;
          request.payload.offer_type = offer_type || (seller_sku || {}).offer_type;
          request.payload.offer_acronym = offer_acronym || (seller_sku || {}).offer_acronym;
          payload_added = true;
