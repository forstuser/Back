...
class ShopEarnAdaptor {
  ...
  async retrieveSKUs(options) {
    try {
      ...
      let seller,
          seller_skus = [],
          seller_categories = [];
      category_id = !title ? (category_id || '').trim().split(',').filter(item => !!item) : [];
      main_category_id = !title ? (main_category_id || '').trim().split(',').filter(item => !!item) : [];
      brand_ids = (brand_ids || '').trim().split(',').filter(item => !!item);
      sub_category_ids = !title ? (sub_category_ids || '').trim().split(',').filter(item => !!item) : [];
      ...
      const seller_main_categories = seller_categories.length > 0 ? seller_categories.map(item => {
        if (item.category_brands.length > 0) {
          item.category_brands.forEach(cbItem => sub_category_ref.push({ ref_id: cbItem.category_4_id }));
        }
      ...
      if (title) {
        ...
        let sub_categories = await this.modals.categories.findAll({
          ...
          where: JSON.parse(JSON.stringify({
            ...
            }, $or: sub_category_ref.length > 0 ? _lodash2.default.uniq(sub_category_ref) : undefined
      ...
      const $or = !title ? {
        ...
        title: { $iLike: `%${title || ''}%` },
        id: _lodash2.default.includes(main_category_id, _main2.default.MILK_SKU_MAIN_CATEGORY) && category_id.length === 0 && _main2.default.MILK_SKU_IDS.length > 0 ? _main2.default.MILK_SKU_IDS : undefined
      } : JSON.parse(JSON.stringify({
        sub_category_id: sub_category_id_list.length > 0 ? sub_category_id_list : undefined, title: {
          $or: JSON.parse(JSON.stringify([...forth_title, { $iLike: first_title }, ...second_title, ...third_title]))
      ...
      if (seller_id) {
        sku_measurement_attributes.push([this.modals.sequelize.literal(`(select offer_discount from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}')`), 'offer_discount'], [this.modals.sequelize.literal(`(select title from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}')`), 'offer_title'], [this.modals.sequelize.literal(`(select offer_type from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}')`), 'offer_type'], [this.modals.sequelize.literal(`(Select acronym from table_sku_measurement as measure where measure.id in (select sku_measurement_type from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}'))`), 'offer_acronym'], [this.modals.sequelize.literal(`(select seller_mrp from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id})`), 'seller_mrp']);
      }
      ...
      const sku_options = JSON.parse(JSON.stringify(seller_main_categories.length > 0 && !title ? {
        ...
        sub_category_id: sub_category_ids.length > 0 ? sub_category_ids : undefined, id: !title ? id ? _lodash2.default.uniq(id) : seller_sku_ids && seller_sku_ids.length > 0 ? _lodash2.default.uniq(seller_sku_ids) : undefined : undefined, status_type: 1,
      ...
      const sku_measurements = await this.retrieveSKUMeasurements({
        options: {
          where: JSON.parse(JSON.stringify({
            status_type: 1, id: !title && seller_sku_measurement_ids && seller_sku_measurement_ids.length > 0 ? _lodash2.default.uniq(seller_sku_measurement_ids) : undefined,
      ...
      return {
        sku_items: skuItems.map(item => {
          item.sku_measurements = sku_measurements.filter(measureItem => measureItem.sku_id === item.id).map(measureItem => {
            measureItem.offer_discount = measureItem.offer_discount || 0;
            return measureItem;
          });
          return item;
        }).filter(item => item.sku_measurements && item.sku_measurements.length > 0).map(item => {
          item.seller_id = seller_id;
          if (item.category_id.toString() === _main2.default.MILK_SKU_CATEGORY.toString()) {
            item.sku_measurements = item.sku_measurements.map(measureItem => {
              measureItem.cashback_percent = parseInt((_main2.default.MILK_SKU_CASH_BACK_PERCENT || 0).toString());
              return measureItem;
            });
          }
  ...
  async retrieveSellerSKUs(options) {
    try {
      ...
      measurement_values = (measurement_values || '').trim().split(',').filter(item => !!item);
      measurement_types = (measurement_types || '').trim().split(',').filter(item => !!item);
      const sku_measurement_attributes = ['id', 'pack_numbers', 'measurement_type', 'measurement_value', [this.modals.sequelize.literal('(select acronym from table_sku_measurement as measurement where measurement.id = "sku_measurements".measurement_type)'), 'measurement_acronym'], [this.modals.sequelize.literal(`(select title from table_seller_offers as offer where offer.sku_id = "sku_measurements".sku_id and offer.sku_measurement_id = "sku_measurements".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}')`), 'offer_title'], [this.modals.sequelize.literal(`(select offer_type from table_seller_offers as offer where offer.sku_id = "sku_measurements".sku_id and offer.sku_measurement_id = "sku_measurements".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}')`), 'offer_type'], [this.modals.sequelize.literal(`(select offer_discount from table_seller_offers as offer where offer.sku_id = "sku_measurements".sku_id and offer.sku_measurement_id = "sku_measurements".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}')`), 'offer_discount'], [this.modals.sequelize.literal(`(Select acronym from table_sku_measurement as measure where measure.id in (select sku_measurement_type from table_seller_offers as offer where offer.sku_id = "sku_measurements".sku_id and offer.sku_measurement_id = "sku_measurements".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}'))`), 'offer_acronym'], [this.modals.sequelize.literal(`(select seller_mrp from table_seller_offers as offer where offer.sku_id = "sku_measurements".sku_id and offer.sku_measurement_id = "sku_measurements".id and "offer".seller_id = ${seller_id})`), 'seller_mrp'], 'mrp', 'bar_code', 'cashback_percent'];
      const sku_attributes = [[this.modals.sequelize.literal('(select category_name from categories as category where category.category_id = sku.sub_category_id)'), 'sub_category_name'], 'brand_id', 'category_id', 'sub_category_id', 'main_category_id', 'title', 'id', 'priority_index'];
      const sku_options = seller_main_categories.length > 0 ? {
        ...
        sub_category_id: sub_category_ids.length > 0 ? sub_category_ids : undefined, title, id: {
          $and: {
            $in: seller_sku_ids && seller_sku_ids.length > 0 ? seller_sku_ids : undefined, $ne: sku_id
          }
        }
      } : {
        status_type: 1,
      ...
      return skuItems.map(item => item.toJSON()).filter(item => item.sku_measurements && item.sku_measurements.length > 0).map(item => {
        item.sku_measurements = _lodash2.default.sortBy(item.sku_measurements, [measureItem => {
          switch (measureItem.measurement_type) {
            case 2:
              return parseFloat(measureItem.measurement_value || 0) * 1000;
    ...
    try {
        ...
        if (seller) {
          if (seller.is_data_manually_added) {
            ...
            categories_data = await this.modals.seller_provider_type.findAll({
              ...
              attributes: ['sub_category_id', 'category_brands', 'seller_id', 'provider_type_id', [this.modals.sequelize.literal('(Select priority_index from categories as category where category.category_id = seller_provider_type.sub_category_id)'), 'priority_index'], 'category_4_id', 'brand_ids']
            });
            seller_categories = categories_data.map(item => item.toJSON());
          }
        }
      ...
      if (seller_categories.length <= 0) {
        const skuItems = await this.modals.sku.findAll({
          where: JSON.parse(JSON.stringify({
            status_type: 1,
            id: seller_sku_ids && seller_sku_ids.length > 0 ? _lodash2.default.uniq(seller_sku_ids) : undefined
          })),
          order: [[this.modals.sequelize.literal('(Select priority_index from categories as category where category.category_id = sku.main_category_id)')]],
          attributes: ['main_category_id', 'category_id', [this.modals.sequelize.literal('(Select priority_index from categories as category where category.category_id = sku.main_category_id)'), 'priority_index'], 'brand_id'],
      ...
      return _lodash2.default.orderBy(seller_categories, ['priority_index']);
    } catch (e) {
      throw e;
    }
  }
    ...
    try {
      ...
      let skuItems = await this.modals.sku.findAll({
        ...
        include: [{
          ...
          exclude: ['status_type', 'updated_by', 'updated_at', 'created_at', 'image_code', 'image_name']
        }, order: [['id']]
      });
  ...
  async retrieveSKUMeasurements(parameters) {
    ...
    if (seller_id) {
      sku_measurement_attributes.push([this.modals.sequelize.literal(`(select title from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}' order by created_at desc limit 1)`), 'offer_title'], [this.modals.sequelize.literal(`(select offer_type from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}' order by created_at desc limit 1)`), 'offer_type'], [this.modals.sequelize.literal(`(select offer_discount from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}' order by created_at desc limit 1)`), 'offer_discount'], [this.modals.sequelize.literal(`(Select acronym from table_sku_measurement as measure where measure.id in (select sku_measurement_type from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} and "offer".end_date >= '${(0, _moment2.default)().format('YYYY-MM-DD')}' order by created_at desc limit 1))`), 'offer_acronym'], [this.modals.sequelize.literal(`(select seller_mrp from table_seller_offers as offer where offer.sku_id = "sku_measurement".sku_id and offer.sku_measurement_id = "sku_measurement".id and "offer".seller_id = ${seller_id} order by created_at desc limit 1)`), 'seller_mrp']);
    }
  ...
  async retrieveReferenceData() {
    try {
      let [main_categories, categories, /*sub_categories,*/measurement_types] = await _bluebird2.default.all([this.modals.categories.findAll({
        ...
        }, order: [['priority_index']], attributes: [['category_id', 'id'], ['category_name', 'title'], 'ref_id', 'priority_index', [this.modals.sequelize.literal('(select count(*) from table_sku_global as sku where sku.main_category_id = "categories".category_id)'), 'sku_counts'], ['category_level', 'level']]
      }), this.modals.categories.findAll({
        where: { category_level: 4, status_type: 1 },
        order: [['priority_index']], attributes: [['category_id', 'id'], ['category_name', 'title'], 'ref_id', 'priority_index', [this.modals.sequelize.literal('(select count(*) from table_sku_global as sku where sku.category_id = "categories".category_id)'), 'sku_counts'], ['category_level', 'level']]
      }),
      /*this.modals.categories.findAll(
          {
  ...
  async retrieveCashBackTransactions(options) {
    try {
      ...
      const [reasons, transaction_detail] = await _bluebird2.default.all([this.categoryAdapter.retrieveReasons({ where: { query_type: [1, 3] }, order: [['id']] }), this.modals.cashback_jobs.findAll({
        where: JSON.parse(JSON.stringify({
          seller_id, user_id, $or: [{ admin_status: { $notIn: [2, 9] } }, { $and: { admin_status: { $eq: 9 }, reason_id: { $not: null } } }]
        })),
        include: [{
  ...
  async createUserSKUWishList(reply, request, user_id) {
    ...
    try {
      const [userSKUWishList, seller_sku] = await _bluebird2.default.all([this.retrieveUserSKUs({
        where: { user_id },
        attributes: ['wishlist_items', 'past_selections']
      }), seller_id && id && sku_measurement_id ? this.sellerAdapter.retrieveSellerOffer({
        where: {
      ...
      if (id) {
        ...
        if (!payload_added) {
          request.payload.offer_discount = (seller_sku || {}).offer_discount || 0;
          wishlist_items.push(JSON.parse(JSON.stringify(_lodash2.default.omit(request.payload, ['status_type', 'updated_by', 'created_at', 'updated_at']))));
        }
      } else {
        ...
        wishlist_items.push(JSON.parse(JSON.stringify(_lodash2.default.omit(userSku, ['status_type', 'updated_by', 'created_at', 'updated_at']))));
      }
      if (seller_id) {
        wishlist_items = wishlist_items.filter(item => item.seller_id === seller_id || !item.seller_id);
      }
