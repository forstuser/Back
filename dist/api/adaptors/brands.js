...
class BrandAdaptor {
  ...
  async retrieveOrUnlinkBrandOffers(options) {
    let seller_offer = await this.modals.seller_offers.findOne({ where: options });
    if (seller_offer && (options.id && seller_offer.id.toString() === options.id.toString() || options.sku_id && seller_offer.sku_id.toString() === options.sku_id.toString() && options.sku_measurement_id && seller_offer.sku_measurement_id.toString() === options.sku_measurement_id.toString())) {
      ...
      await seller_offer.updateAttributes({ brand_offer_id: null, status_type: 2 });
    }
    return seller_offer ? seller_offer.toJSON() : undefined;
  }

  async createBrandOfferRequestForSeller(options) {
    let seller_offer = await this.modals.seller_offer_request.findOne({ where: options });
    if (!seller_offer) {
