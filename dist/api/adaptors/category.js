...
class CategoryAdaptor {
  ...
  async retrieveCategories(parameters) {
    ...
    const result = await this.modals.categories.findAll({
      where: options,
      attributes: [['category_id', 'id'], ['category_name', 'default_name'], [`${language ? `category_name_${language}` : `category_name`}`, 'name'], [this.modals.sequelize.fn('CONCAT', '/categories/', this.modals.sequelize.literal('"categories"."category_id"'), '/images/1'), 'categoryImageUrl'], 'priority_index'],
      order: ['category_id']
    });
