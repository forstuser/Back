  ...
  async handleDelayedCashBack(options) {
    ...
    const delayed_cash_back_count = await this.modals.user_wallet.count({
      where: {
        ...
        created_at: {
          $between: [(0, _moment2.default)().utcOffset(330).startOf('day').format(''), (0, _moment2.default)().utcOffset(330).format('')]
        }
      }
    });
    ...
    if (delayed_cash_back_count === 0) {
      const [delayed_user_cash_back, deducted_seller_cash_back] = await Promise.all([this.modals.user_wallet.create(JSON.parse(JSON.stringify({
        status_type: 16, order_id, cashback_source: 10,
        amount, transaction_type: 1, user_id, seller_id
      }))), this.modals.cashback_wallet.create(JSON.parse(JSON.stringify({
        status_type: 16, order_id, cashback_source: 10,
        amount, transaction_type: 1, user_id, seller_id
      }))), this.modals.seller_wallet.create(JSON.parse(JSON.stringify({
