...
class OfferAdaptor {
  ...
  async getOfferCategories(options) {
    ...
    const offerInclude = {
      ...
      as: 'main_offers', where: {
        status_type: 1, date_end: { $gte: (0, _moment2.default)().format() }
    ...
    return await this.retrieveOfferCategories({
      ...
      attributes: [[this.modals.sequelize.literal(`count(${offerInclude.as})`), 'offer_counts'], 'id', 'category_level', 'category_name', 'category_image_name'],
      order: [['category_name']],
  ...
  async getOfferList(options) {
    ...
    const offerInclude = {
      ...
      where: { status_type: 1, date_end: { $gte: (0, _moment2.default)().format() } },
    ...
    if (selected_category.category_level === 1) {
      const categories = await this.retrieveOfferCategories({
        where: { ref_id: id },
        include: [offerInclude],
        attributes: ['id', 'category_level', 'category_name', 'category_image_name', [this.modals.sequelize.literal(`count(${offerInclude.as})`), 'offer_counts']],
        group: ['"offerCategories"."id"']
      });

      offers = await Promise.all(categories.map(async item => {
        const offerOptions = {
        ...
        return await this.retrieveOffers({ where: offerOptions, offset, limit });
      }));
