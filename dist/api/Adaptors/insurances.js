...
class InsuranceAdaptor {
  ...
  createInsurances(values) {
    return this.modals.insurances.create(values).then(result => result.toJSON());
  }
  ...
  findCreateInsuranceBrand(values) {
    ...
    return this.modals.insuranceBrands.findOne({
      ...
      if (!result) {
        return this.modals.insuranceBrands.create(_lodash2.default.omit(values, 'category_id'));
      }
