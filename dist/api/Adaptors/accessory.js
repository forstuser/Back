...
class AccessoryAdaptor {
  ...
  async getAccessoryCategories(options) {
    try {
      ...
      return categories.map(item => {
        item.accessories = _lodash2.default.orderBy(item.accessories, ['priority', 'title'], ['asc', 'asc']);
  ...
  async getAccessoriesList(options) {
    try {
      ...
      accessoryCategories = accessoryCategories.map(item => {
        item.accessory_items = _lodash2.default.orderBy(accessoryProducts.filter(apItem => apItem.accessory_id === item.id && !apItem.isOutOfStock), ['bb_class', 'affiliate_type'], ['asc', 'asc']);
        return item;
      });
      return categories.map(item => {
        item.accessories = _lodash2.default.orderBy(accessoryCategories.filter(acItem => acItem.category_id === item.category_id), ['priority', 'title'], ['asc', 'asc']);
  ...
  async retrieveAccessoryCategoryProducts(parameters) {
    try {
      ...
      const accessoryOptions = {
        ...
        $and: [{
          id: {
            $in: this.modals.sequelize.literal(`(SELECT "accessory_id" FROM "table_accessory_products" AS "accessory_items" WHERE ("accessory_items"."title" != '' AND "accessory_items"."title" IS NOT NULL 
                ...
                AND ("accessory_items"."details"#>>'{image}') IS NOT NULL) AND (("accessory_items"."details"#>>'{name}') != '' AND ("accessory_items"."details"#>>'{name}') IS NOT NULL)
                 AND (("accessory_items"."details"#>>'{price}') != '' AND ("accessory_items"."details"#>>'{price}') IS NOT NULL))) AND "accessory_items"."accessory_id" = "table_accessory_categories"."id" ${accessory_type_id ? `or (accessory_type_id in (${accessory_type_id.toString()}) and accessory_id in (${model_accessory.toString()}))` : `and accessory_id not in (${model_accessory.toString()})`})`)
          }
        }]
      };
      ...
      if (categoryid) {
        ...
        return await _bluebird2.default.all([this.retrieveAccessoryCategories({
          ...
          where: categoryNameOptions, include: [{
            model: this.modals.products,
            as: 'products', where: productOptions, attributes: ['brand_id', 'main_category_id', 'category_id', 'product_name', 'id', 'model'], required: false
          }], attributes: ['category_id', 'category_name']
        })]);
      }
      const products = await this.retrieveProducts({
        where: productOptions, attributes: ['brand_id', 'main_category_id', 'category_id', 'product_name', 'id', 'model']
      });
  ...
  async getOrderHistory(options) {
    try {
      ...
      let accessory_product_ids = [],
          product_ids = [],
          payment_mode_ids = [];
      if (transactions.length > 0) {
        accessory_product_ids = transactions.map(transaction => transaction.accessory_product_id);
        product_ids = transactions.map(transaction => transaction.product_id);
        payment_mode_ids = transactions.map(transaction => transaction.payment_mode_id);
      }
      ...
      product_ids.length > 0 ? this.retrieveProducts({ where: JSON.parse(JSON.stringify({ id: product_ids })) }) : [],
      // payment mode is also there.
      // add the payment mode in the result of each transactions well.
      payment_mode_ids.length > 0 ? this.retrievePaymentMode({ where: JSON.parse(JSON.stringify({ id: payment_mode_ids })) }) : []]);
  ...
  async createTransaction(options, reply, request) {
    ...
    try {
      let seller = {};
      const transactionDetail = await this.modals.table_transaction.findOne({ where: { transaction_id } });
      if (!transactionDetail) {
        ...
        }), this.modals.products.findOne({ where: { id: product_id } }), this.modals.table_accessory_products.findOne({ where: { id: accessory_product_id } })]);
        ...
        return reply.response({
          status: true,
          result: await _bluebird2.default.all([result, this.productAdapter.createEmptyProduct({
