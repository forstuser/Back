...
class NearByAdaptor {
	...
	retrieveNearBy(location, geoLocation, professionIds, reply, userId, request) {
		...
		return this.filterNearByProfessional(professionIds.split('[')[1].split(']')[0].split(',').filter(Boolean), userId).then(result => {
			const finalResult = [];
				...
				if (origins.length > 0 && destinations.length > 0) {
					googleMapsClient.distanceMatrix({
						...
						for (let i = 0; i < userWithOrigins.length; i += 1) {
							const tempMatrix = matrix.status === 200 && matrix.json ? matrix.json.rows[0].elements[i] : {};
