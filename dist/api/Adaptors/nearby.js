...
var NearByAdaptor = function () {
	...
	_createClass(NearByAdaptor, [{
		key: 'retrieveNearBy',
		value: function retrieveNearBy(location, geoLocation, professionIds, reply, userId, request) {
			var origins = [];
			var destinations = [];
			if (geoLocation) {
			...
			this.filterNearByProfessional(professionIds.split('[')[1].split(']')[0].split(',').filter(Boolean), userId).then(function (result) {
				var finalResult = [];
				var userWithOrigins = [];
				if (result.length > 0) {
					...
					if (origins.length > 0 && destinations.length > 0) {
						googleMapsClient.distanceMatrix({
							...
							for (var i = 0; i < userWithOrigins.length; i += 1) {
								var tempMatrix = matrix.status === 200 && matrix.json ? matrix.json.rows[0].elements[i] : {};
