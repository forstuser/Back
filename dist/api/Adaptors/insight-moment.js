  ...
  _createClass(InsightAdaptor, [
    {
            ...
            then(function(result) {
              var categoryData = !(minDate || maxDate) ? {
                weeklyData: result[0].map(function(periodItem) {
                  ...
                  categoryPeriodItem.totalAmount = categoryPeriodItem.totalAmount ||
                      0;
                  categoryPeriodItem.totalTax = categoryPeriodItem.totalTax ||
                      0;
              ...
              if (minDate || maxDate) {
                ...
                var totalAmounts = sumProps(categoryData.customDateData,
                    'totalAmount');
                var totalTaxes = sumProps(categoryData.customDateData,
                    'totalTax');
                return {
                  status: true,
              ...
              var totalWeeklyAmounts = sumProps(categoryData.weeklyData,
                  'totalAmount');
              var totalWeeklyTaxes = sumProps(categoryData.weeklyData,
                  'totalTax');
              var totalYearlyAmounts = sumProps(categoryData.yearlyData,
                  'totalAmount');
              var totalYearlyTaxes = sumProps(categoryData.yearlyData,
                  'totalTax');
              var totalMonthlyAmounts = sumProps(categoryData.monthlyData,
                  'totalAmount');
              var totalMonthlyTaxes = sumProps(categoryData.monthlyData,
                  'totalTax');
              return {
                status: true,
                message: 'Insight restore successful',
                notificationCount: 0,
                categoryData: categoryData,
                weekStartDate: _moment2.default.utc().
                    startOf('week').
                    startOf('week').
                    format('YYYY-MM-DD'),
                monthStartDate: _moment2.default.utc().
                    startOf('week').
                    startOf('month').
                    format('YYYY-MM-DD'),
                weekEndDate: _moment2.default.utc().
                    startOf('week').
                    endOf('week').
                    format('YYYY-MM-DD'),
                monthLastDate: _moment2.default.utc().
                    startOf('week').
                    endOf('month').
                    format('YYYY-MM-DD'),
                yearStartDate: _moment2.default.utc().
                    startOf('week').
                    startOf('year').
                    format('YYYY-MM-DD'),
      ...
      value: function prepareCategoryData(user, minDate, maxDate) {
        return !(minDate || maxDate) ? Promise.all([
          this.modals.categories.findAll({
            ...
            attributes: [
              ...
              [
                ...
                'cURL'],
              [
                this.modals.sequelize.fn('SUM', this.modals.sequelize.col(
                    '`products`.`value_of_purchase`')),
                'totalAmount'],
              [
                this.modals.sequelize.fn('SUM',
                    this.modals.sequelize.col('`products`.`taxes`')),
                'totalTax']],
              ...
              include: [
                {
                  ...
                  where: {
                    ...
                    purchase_date: {
                      $gte: minDate ?
                          (0, _moment2.default)(minDate).utc().startOf('day') :
                          _moment2.default.utc().
                              startOf('week').
                              startOf('month'),
                      $lte: maxDate ?
                          (0, _moment2.default)(maxDate).utc().endOf('day') :
                          _moment2.default.utc().startOf('week').endOf('month'),
                    },
                  },
                  attributes: [],
                  include: [
          ...
          attributes: [
            ...
            [
              this.modals.sequelize.fn('SUM',
                  this.modals.sequelize.col('`products`.`value_of_purchase`')),
              'totalAmount'],
            [
              this.modals.sequelize.fn('SUM',
                  this.modals.sequelize.col('`products`.`taxes`')),
      ...
      value: function prepareCategoryInsight(user, request) {
        ...
        var promisedQuery = Promise.all([
          this.fetchProductDetails(user, masterCategoryId, minDate, maxDate),
          this.modals.categories.findOne({
            ...
            attributes: [
              ...
              [
                this.modals.sequelize.fn('CONCAT', 'categories/',
                    this.modals.sequelize.col('category_id'), '/image/'),
                'cImageURL']],
          })]);
        return promisedQuery.then(function(result) {
          ...
          result[0].map(function(item) {
            ...
            var index = distinctInsight.findIndex(function(distinctItem) {
              return (0, _moment2.default)(distinctItem.date).valueOf() ===
                  (0, _moment2.default)(product.consumerBill.purchaseDate).
                      utc().
                      endOf('day').
                      valueOf();
            });
            if (index === -1) {
              distinctInsight.push({
                ...
                month: monthArray[(0, _moment2.default)(
                    product.consumerBill.purchaseDate).utc().month()],
                monthId: (0, _moment2.default)(
                    product.consumerBill.purchaseDate).utc().month() + 1,
                date: (0, _moment2.default)(product.consumerBill.purchaseDate).
                    utc().
                    endOf('day'),
                week: weekAndDay(
                    (0, _moment2.default)(product.consumerBill.purchaseDate).
          ...
          productList.sort(function(a, b) {
            return new Date(b.consumerBill.purchaseDate) -
                new Date(a.consumerBill.purchaseDate);
          });
          var productListWeekly = productList.filter(function(item) {
            return (0, _moment2.default)(item.consumerBill.purchaseDate).
                    endOf('day').
                    valueOf() >= _moment2.default.utc().
                    startOf('week').
                    startOf('month').
                    valueOf() &&
                (0, _moment2.default)(item.consumerBill.purchaseDate).
                    endOf('day').
                    valueOf() <=
                _moment2.default.utc().startOf('week').endOf('month').valueOf();
          }).slice(0, 10);
          var productListMonthly = productList.filter(function(item) {
            return (0, _moment2.default)(item.consumerBill.purchaseDate).
                ...
                (0, _moment2.default)(item.consumerBill.purchaseDate).
                    endOf('day').
                    valueOf() <=
                _moment2.default.utc().startOf('week').endOf('year').valueOf();
          }).slice(0, 10);
          ...
          var insightData = distinctInsightTemp.filter(function(item) {
            return (0, _moment2.default)(item.consumerBill.purchaseDate).
                ...
                (0, _moment2.default)(item.consumerBill.purchaseDate).
                    endOf('day').
                    valueOf() <=
                _moment2.default.utc().startOf('week').endOf('week').valueOf();
          });
          ...
          var insightWeekly = distinctInsightWeekly.filter(function(item) {
            return (0, _moment2.default)(item.date).endOf('day').valueOf() >=
                _moment2.default.utc().
                    startOf('week').
                    startOf('month').
                    valueOf() &&
                (0, _moment2.default)(item.date).endOf('day').valueOf() <=
                _moment2.default.utc().startOf('week').endOf('month').valueOf();
          });
