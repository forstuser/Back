...
class SellerAdaptor {
  ...
  async retrieveSellerConsumers(seller_id, mobile_no, offer_id) {
    ...
    return await this.orderUserByLocation(latitude, longitude, city, result.map(item => {
      ...
      item.loyalty_total = (item.loyalty_total || 0) - (item.redeemed_loyalty || 0);
      item.credit_total = (item.credit_total || 0) - (item.redeemed_credits || 0);
      item.addresses = (item.addresses || []).find(aItem => aItem.address_type === 1);
      ...
      if (item.addresses) {
        ...
        item.address = `${address_line_1}${address_line_2 ? ` ${address_line_2}` : ''},${locality_name},${city_name},${state_name}-${pin_code}`.split('null').join(',').split('undefined').join(',').split(',,').join(',').split(',,').join(',');
      }
      if (offer_id) {
