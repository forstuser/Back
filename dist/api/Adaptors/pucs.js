...
class PUCAdaptor {
  ...
  async updatePUCPeriod(options, purchase_date, new_purchase_date) {
    const result = await this.modals.pucs.findAll({ where: options, order: [['document_date', 'ASC']] });
    let document_date = new_purchase_date;
    let pucExpiryDate;
    return Promise.all(result.map(item => {
      const pucItem = item.toJSON();
      const id = pucItem.id;
      if (_moment2.default.utc(pucItem.effective_date).startOf('days').isSame(_moment2.default.utc(purchase_date).startOf('days')) || _moment2.default.utc(pucItem.effective_date).startOf('days').isBefore(_moment2.default.utc(new_purchase_date).startOf('days'))) {
        ...
        pucItem.expiry_date = _moment2.default.utc(new_purchase_date, _moment2.default.ISO_8601).add(_moment2.default.utc(pucItem.expiry_date, _moment2.default.ISO_8601).add(1, 'days').diff(_moment2.default.utc(purchase_date, _moment2.default.ISO_8601), 'months', true), 'months').subtract(1, 'days');
      ...
      } else if (_moment2.default.utc(pucItem.effective_date).startOf('days').isSame(_moment2.default.utc(pucExpiryDate).startOf('days'))) {
  ...
  async deletePUCs(id, user_id) {
    ...
    if (result) {
      ...
      }), this.modals.pucs.destroy({ where: { id, user_id } }), result.copies && result.copies.length > 0 ? this.modals.jobCopies.update({ status_type: 3, updated_by: user_id }, { where: { id: result.copies.map(item => item.copyId) } }) : undefined]);
    }
