        ...
        var categoryData = !(minDate || maxDate) ? {
          weeklyData: result.map(function (item) {
            var expenses = item.expenses.filter(function (item) {
              return item.purchaseDate >= (0, _moment2.default)(_moment2.default.utc().subtract(6, 'd')).utc().startOf('d') && item.purchaseDate <= _moment2.default.utc().endOf('d');
            });
          ...
          monthlyData: result.map(function (item) {
            var expenses = item.expenses.filter(function (item) {
              return item.purchaseDate >= (0, _moment2.default)().utc().startOf('month') && item.purchaseDate <= _moment2.default.utc().endOf('month');
            });
            var totalAmount = _shared2.default.sumProps(expenses, 'value');
            var totalTax = _shared2.default.sumProps(expenses, 'taxes');
            return {
          ...
          yearlyData: result.map(function (item) {
            var expenses = item.expenses.filter(function (item) {
              return item.purchaseDate >= (0, _moment2.default)().utc().startOf('year') && item.purchaseDate <= _moment2.default.utc().endOf('year');
            });
            var totalAmount = _shared2.default.sumProps(expenses, 'value');
            var totalTax = _shared2.default.sumProps(expenses, 'taxes');
            return {
          ...
          customDateData: result.map(function (item) {
            var expenses = item.expenses.filter(function (item) {
              return item.purchaseDate >= (0, _moment2.default)(minDate).utc().startOf('d') && item.purchaseDate <= (0, _moment2.default)(maxDate).utc().endOf('d');
            });
            var totalAmount = _shared2.default.sumProps(expenses, 'value');
            var totalTax = _shared2.default.sumProps(expenses, 'taxes');
            return {
    ...
    value: function prepareCategoryData(user, options) {
      ...
      return Promise.all([this.categoryAdaptor.retrieveCategories(categoryOption), this.productAdaptor.retrieveProducts(productOptions), this.amcAdaptor.retrieveAMCs(productOptions), this.insuranceAdaptor.retrieveInsurances(productOptions), this.repairAdaptor.retrieveRepairs(productOptions), this.warrantyAdaptor.retrieveWarranties(productOptions)]).then(function (results) {
        return results[0].map(function (categoryItem) {
          ...
          category.expenses = [].concat(_toConsumableArray(products), _toConsumableArray(amcs), _toConsumableArray(insurances), _toConsumableArray(repairs), _toConsumableArray(warranties)) || [];
      ...
      return this.prepareCategoryData(user, { category_id: masterCategoryId }).then(function (result) {
        ...
        result[0].expenses.map(function (item) {
          ...
          var index = distinctInsight.findIndex(function (distinctItem) {
            return (0, _moment2.default)(distinctItem.purchaseDate).startOf('day').diff((0, _moment2.default)(expense.purchaseDate).startOf('day'), 'days') === 0;
          });
          if (index === -1) {
            distinctInsight.push({
              value: expense.value,
              month: monthArray[(0, _moment2.default)(expense.purchaseDate).month()],
              monthId: (0, _moment2.default)(expense.purchaseDate).month() + 1,
              purchaseDate: (0, _moment2.default)(expense.purchaseDate),
              week: weekAndDay((0, _moment2.default)(expense.purchaseDate)).monthWeek,
              day: weekAndDay((0, _moment2.default)(expense.purchaseDate)).day,
              tax: expense.taxes
            });
          } else {
        ...
        var productList = _lodash2.default.chain(result[0].expenses).orderBy(['purchaseDate'], ['asc']);
