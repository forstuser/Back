...
class JobAdaptor {
  ...
  async cashBackApproval(options, seller_name) {
    ...
    let home_delivery_limit = user_limit_rules.find(item => item.rule_type === 7 && item.seller_id);
    home_delivery_limit = home_delivery_limit && user_default_limit_rules.find(item => item.rule_type === 7);
  ...
  async cashBackRedemption(options) {
    ...
    const total_amount = _lodash2.default.sumBy(seller_cashback, 'amount');
