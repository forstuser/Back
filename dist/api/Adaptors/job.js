...
class JobAdaptor {
  ...
  async retrieveJobDetail(id, isUpload) {
    const jobResult = await Promise.all([this.modals.jobs.findById(id), this.modals.products.findOne({ where: { job_id: id }, attributes: ['id'] }), this.modals.jobCopies.findAll({ where: { job_id: id } })]);
