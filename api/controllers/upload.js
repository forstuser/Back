...
class UploadController {
  ...
  static uploadUserImage(request, reply) {
    ...
    if (!user) {
      ...
      fsImplUser.writeFile(fileName, fileData._data,
    ...
    return modals.consumerBills.create({
      ...
      if (Array.isArray(fileData)) {
        ...
        const fileUploadPromises = fileData.map((elem) => {
          ...
          const fileName = `${user.ID}-${result.bill_id}-${moment().
              valueOf()}.${(fileType)
              ? fileType.toString()
              : fileTypeData.ext}`;
          ...
          // const file = fs.createReadStream();
          return fsImpl.writeFile(fileName, elem._data,
        ...
        Promise.all(fileUploadPromises).then((fileResult) => {
          const promisedQuery = fileResult.map((elem, index) => {
            const ret = {
              ...
              bill_copy_type: (fileTypes[index])
            ...
            modals.consumerBills.count({
              ...
              if (billCount === 1) {
                notificationAdaptor.sendMailOnDifferentSteps(
                    'It’s good to see you start building your eHome', user.email ||
                    user.email_id, user, 2);
              } else {
                notificationAdaptor.sendMailOnDifferentSteps(
                    'We have received your bill, soon it will be available in your eHome', user.email ||
                    user.email_id, user, 3);
              }
            });
          }
        ...
        if (fileType && !isFileTypeAllowed(fileType)) {
          ...
          fsImpl.writeFile(fileName, fileData._data,
            ...
            console.log(fileResult);
            modals.billCopies.create(ret).then(billResult => {
              if (user.email || user.email_id) {
                modals.consumerBills.count({
                  where: {
                    uploaded_by: user.ID,
    ...
    if (!request.pre.forceUpdate) {
      modals.billCopies.findOne({
        ...
        if (result && result.bill_copy_name) {
          fsImpl.readFile(result.bill_copy_name, 'utf8').
