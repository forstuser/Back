        ...
        return this.modals.productBills.findOne({
            ...
            if (result) {
                ...
                return Promise.all([this.modals.brandDetails.findAll({
                    ...
                    if (product.consumerBill.productOnlineSeller) {
                        product.consumerBill.productOnlineSeller.sellerDetails = mappableResult[2].map(item => item.toJSON());
                    }

                    if (product.consumerBill.productOfflineSeller) {
                        product.consumerBill.productOfflineSeller.sellerDetails = mappableResult[3].map(item => item.toJSON());
                    }
